...
int balong_fb_init_varinfo(struct balong_fb_data_type *balongfd, int pixel_fmt)
{
    ...
    pr_info("%s,fbi->screen_base:0x%pK \n",__func__,fbi->screen_base);
...
STATIC int balong_fb_release(struct fb_info *info, int user)
{
    ...
    balongfb_logi(" enter, balongfd->ref_cnt = %d!\n", balongfd->ref_cnt);

    balongfd->ref_cnt--;
    if (!balongfd->ref_cnt) {
        /* DTS2012092606491 init var_screeninfo and fix_screeninfo */
...
/* BEGIN PN:DTS2014062806461 , Added by z00280572, add read lcd reg feature 2014/07/02*/
	sema_init(&balongfd->mipi_read_sem, 1);
/* END PN:DTS2014062806461 , Added by z00280572, add read lcd reg feature 2014/07/02*/
