...
struct hi6521_device_info {
    ...
    /* END PN:DTS2014083001448, Added by l00189337, 2014/8/28*/
...
/*BEGIN PN:DTS2014111404882  Added by m00264465 20141119*/
#ifdef CONFIG_HUAWEI_HLTHERM_CHARGING
STATIC unsigned int charge_temp_protected = 0;
#endif
/*END PN:DTS2014111404882  Added by m00264465 20141119*/
...
/*DTS:DTS2014081907069  d00273657 2014-09-03 begin*/
STATIC u32 dpm_switch_enable = 0;
/*DTS:DTS2014081907069  d00273657 2014-09-03 end*/
/*DTS:DTS2014080104882 z00269720 2014-08-06 begin*/
STATIC unsigned int vbus_ovp_flag = 0;
/*DTS:DTS2014080104882 z00269720 2014-08-06 end*/

/* BEGIN PN:DTS2014082908201, Added by l00189337, 2014/9/3*/
#ifdef CONFIG_CHARGER_TYPE_RECHECK
static int work_first_in_since_vbus_intr = 1;
#endif
/* END PN:DTS2014082908201, Added by l00189337, 2014/9/3*/
enum{
...
STATIC void charge_core_safe_policy_handler(struct hi6521_device_info *di)
{
    ...
    if (change)
    {
        ...
        hwlog_info(BASP_TAG"before apply asp, di->ichg:%dmA, di->vterm:%dmV\n", di->ichg, di->vterm);
    }

    tmp_ratio = (asp.cur_ratio < MAX_BATT_CHARGE_CUR_RATIO) ? asp.cur_ratio : MAX_BATT_CHARGE_CUR_RATIO;
    di->ichg = di->ichg <= asp.learned_fcc*tmp_ratio/10 ? di->ichg : asp.learned_fcc*tmp_ratio/10;
    di->vterm = di->vterm <= (vterm_max-asp.volt_dec) ? di->vterm : (vterm_max-asp.volt_dec);

    if (change)
    {
        hwlog_info(BASP_TAG"after apply asp, di->ichg:%dmA, di->vterm:%dmV, tmp_ratio:%d\n", di->ichg, di->vterm, tmp_ratio);
    }
}
...
STATIC void charge_select_charging_current_voltage(struct hi6521_device_info *di)
{
    di->chg_fast_ichg = ((di->chg_fast_ichg < di->ichg) ? di->chg_fast_ichg : di->ichg );
    di->chg_fast_vchg = ((di->chg_fast_vchg < di->vterm) ? di->chg_fast_vchg : di->vterm );

    hwlog_info(BASP_TAG"di->ichg:%dmA, di->vterm:%dmV\n", di->ichg, di->vterm);
    hwlog_info(BASP_TAG"di->chg_fast_ichg:%dmA, di->chg_fast_vchg:%dmV\n", di->chg_fast_ichg, di->chg_fast_vchg);
...
/****************************************************************************
  Function:     hi6521_config_enable_chopper
  Description:  config chopper enable/disable.
  Input:        struct hi6522_device_info,di->wled need setup.
  Output:       NA
  Return:       0:success others:fail
****************************************************************************/
static int hi6521_config_enable_chopper(struct hi6521_device_info *di,int value)
{
    int ret = 0;
    unsigned char val = 0;
    ...
    ret = hi6521_read_byte(di,(u8 *)&val,WLED_REG8_CONTROL);
    val &= HI6XXX_WLED_REG8_CHOOPER_MASK;
    if(ENABLE_CHOOPER == value)
    {
        val |= HI6XXX_WLED_REG8_CHOOPER_ENABLE;
    }

    ret |= hi6521_write_byte(di,val,WLED_REG8_CONTROL);

    up(&di->charger_data_busy_lock);
    hwlog_err("[%s] :hi6521_config_enable_chopper is %d\n", __func__,
             val);
    return ret;
}
/****************************************************************************
  Function:     hi6521_config_enable_ifb1_ifb2
  Description:  config ifb1_ifb2 enable/disable.
  Input:        struct hi6521_device_info,di->wled need setup.
  Output:       NA
  Return:       0:success others:fail
****************************************************************************/
static int hi6521_config_enable_ifb1_ifb2(struct hi6521_device_info *di,int mode)
{
    int ret = 0;
    unsigned char val = 0;
    ...
    ret = hi6521_read_byte(di,(u8 *)&val,WLED_REG5_CONTROL);
    val &= HI6XXX_IFB1_IFB2_ENABLE_MASK;
    val |= mode;

    ret |= hi6521_write_byte(di,(u8)val,WLED_REG5_CONTROL);

    up(&di->charger_data_busy_lock);
    hwlog_err("[%s] :hi6521_config_enable_ifb1_ifb22 is %d\n", __func__,
             val);
    return ret;
}
/****************************************************************************
...
STATIC void hi6521_config_opt_param(struct hi6521_device_info *di)
{
	...
	hi6521_read_byte(di, &chooper, HI6XXX_WLED_REG8);
	chooper &= HI6XXX_WLED_REG8_CHOOPER_ENABLE;
	chooper |= 0x69;//mask chooper_enable
	...
	if ((CHG_V200 == chg_version) && (CHG_V200 == v210_fx))
	{
		...
		hi6521_write_byte(di, chooper, 0x1D);
	}
	else
	{
	    ...
	    if( 0 != customized_config_by_product ){
	        /*Modify flash_bst_vo from 4.8V to 4.7V, need by audio*/
	        hi6521_write_byte(di, 0x08, 0x5f);
...
STATIC void hi6521_start_usb_charger(struct hi6521_device_info *di)
{
    ...
    while(HI6521_VBUS_STAT_UNKNOWM == hi6521_check_charger_present(di)) {
        if (!(timeout--)){
            ...
            /*DTS:DTS2014080105942 z00269720 2014-08-07 begin*/
            events = VCHRG_NOT_CHARGING_EVENT;
            break;
            /*DTS:DTS2014080104882 z00269720 2014-08-07 end*/
        }
        msleep(5);
    }
    ...
    if(di->backup_battery_exist) {
        ...
        /*BEGIN PN:DTS2014122205995  Modify by z00269720 20141222*/
        if(CHARGER_TYPE_BC_USB == di->event)
        {
            di->cin_limit = IINLIM_1000;
            di->chg_fast_ichg = CHG_FAST_ICHG_VALUE(900);
        ...
        /*END PN:DTS2014122205995  Modify by z00269720 20141222*/
    }
...
STATIC void hi6521_charger_update_status(struct hi6521_device_info *di)
{
    ...
    /*DTS:DTS2014080104882 z00269720 2013-08-06 end*/
...
STATIC void hi6521_charger_work(struct work_struct *work)
{
    ...
    charge_core_safe_policy_handler(di);

    charge_select_charging_current_voltage(di);
...
/*DTS:DTS2014102501336  z00269720 2014-10-29 begin*/
static int dpm_check_current_threshold_ma = 40;
/*DTS:DTS2014102501336  z00269720 2014-10-29 end*/
...
STATIC void hi6521_irq_work_handle(struct work_struct *work)
{
    ...
    if (irq_status[1] & CHG_IRQ1_WLED_OVP_38V){
        ...
        wled_ovp_count++;
        hi6521_config_enable_chopper(di,DISABLE_CHOOPER);

        if(WLED_IRQ_ONCE_COUNT == wled_ovp_count)
        {
            hi6521_config_enable_ifb1_ifb2(di,wled_ovp_count);
            hwlog_err("SCHARGER ERROR:CHG_IRQ1_WLED_OVP_38V,wled_ovp_count:%d.\n",wled_ovp_count);
        }
        ...
        {
            hi6521_config_enable_chopper(di,ENABLE_CHOOPER);
            hi6521_config_enable_ifb1_ifb2(di,wled_ovp_count);
            wled_ovp_count = 0;//clear count to zero
    ...
    if (irq_status[1] & CHG_IRQ1_WLED_UVP){
        ...
        {
            hi6521_config_enable_ifb1_ifb2(di,wled_uvp_count);
            hwlog_err("SCHARGER ERROR:CHG_IRQ1_WLED_UVP,wled_uvp_count:%d.\n",wled_uvp_count);
        }
        ...
        else if(WLED_IRQ_THIRD_COUNT == wled_uvp_count)
        {
            hi6521_config_enable_chopper(di,ENABLE_CHOOPER);
            hi6521_config_enable_ifb1_ifb2(di,wled_uvp_count);
            wled_uvp_count = 0;//clear count to zero
            di->wled_en = CHG_POWER_DIS;
            hi6521_config_wled_enable(di);
            hwlog_err("SCHARGER ERROR:CHG_IRQ1_WLED_UVP,wled_uvp_count :%d.\n",wled_uvp_count);
        }
                  ...
                  const char *buf, size_t count)
{
    ...
    /*DTS:DTS2014090404421 l84015384 2014-09-04 begin*/
    di->cin_dpmmV = (unsigned int)val;
    hi6521_config_input_source_reg(di);
    /*DTS:DTS2014090404421 l84015384 2014-09-04 end*/
...
/*DTS:DTS2014091504659  l84015384 2014-09-15 begin*/
...
/*DTS:DTS2014091504659  l84015384 2014-09-15 end*/
                           ...
                           const struct i2c_device_id *id)
{
    ...
    di->vterm                  = di->max_voltagemV;
    di->ichg                   = CHG_FAST_ICHG_MAX ;
