...
VOS_UINT32 At_QrySecuBootFeaturePara( VOS_UINT8 ucIndex )
{
    ...
    return AT_OK;
    /* Modified by Z00306637 for DTS2014120903868, 2014-12-15, end */
}
...
{
    /* Modified by Z00306637 for DTS2014120903868, 2014-12-15, begin */
...
VOS_UINT32 AT_QryRsimPara(VOS_UINT8 ucIndex)
{
    ...
    /* Modified by w00181244 for AT Project, 2011-10-12, Begin*/
    /* 获取 SIM 卡类型和状态 */
    ucCardType   =  pstUsimInfoCtx->enCardType;
    ucCardStatus =  pstUsimInfoCtx->enCardStatus;
    /* Modified by w00181244 for AT Project, 2011-10-12, End*/
...
VOS_UINT32 AT_QryBatVolPara(VOS_UINT8 ucIndex)
{
    ...
    /* Modified by w00181244 for AT Project, 2011-10-15, Begin*/
    /*获取 电池电压值*/
    ...
    /* Modified by w00181244 for AT Project, 2011-10-15, End*/
}
...
VOS_UINT32 AT_QryPhyNumPara(VOS_UINT8 ucIndex)
{
    ...
    /*Modified by w00181244 for AT Project, 2011-10-3, Begin*/
    /* 读取NV项获取IMEI */
    ulRet = AT_GetImeiValue(enModemId, aucAsciiImei);
    /*Modified by w00181244 for AT Project, 2011-10-3, End*/
    ...
    /* Modified by A for DTS2012041104111, 2012-04-19, Begin */
    ...
    /* Modified by A for DTS2012041104111, 2012-04-19, End */
...
TAF_UINT32 At_QryPNNPara (TAF_UINT8 ucIndex)
{
    ...
    /* Modified by f00179208 for DTS2012062202129, 2012-06-25 Begin */
    return AT_OK;
    /* Modified by f00179208 for DTS2012062202129, 2012-06-25 End */
}
...
TAF_UINT32 At_QryOPLPara (TAF_UINT8 ucIndex)
{
    ...
    /* Modified by l00198894 for DTS2012121702841, 2012/12/24, begin */
    return AT_OK;
    /* Modified by l00198894 for DTS2012121702841, 2012/12/24, end */
...
{
    /* Added by c00173809 for AT Project 2011-9-29 Begin */
    VOS_UINT32                           ulRst;
    VOS_UINT8                            ucType;
...
/* Modified by A00165503 for AT Project, 2011-10-17, end */
...
{
    /* Modified by f00179208 for At Project, 2011-10-5, Begin */
    /* 不是查询命令返回参数错误 */
    ...
    /* Modified by f00179208 for At Project, 2011-10-5, End */
...
TAF_UINT32 At_QryU2DiagPara(TAF_UINT8 ucIndex)
{
    ...
    /* Modified by f62575 for DTS2012050704264, 2012-5-10, Begin   */
    /* 读取PID使能NV项 */
    ...
    /* Modified by f62575 for DTS2012050704264, 2012-5-10, end   */
...
TAF_UINT32 At_QryDwinsPara(TAF_UINT8 ucIndex)
{
    ...
    /* Modified by l00171473 for DTS2012082204471, 2012-6-25, begin */

    PS_MEM_SET(&stWins, 0x00, sizeof(stWins));

    /* Modified by l00171473 for DTS2012082204471, 2012-6-25, end */
...
)
{
    ...
    /* Added by lkf58113 for AT Project, 2011-11-28, Begin*/
    /* 调用 LTE 模的接口分支 */ /* FIXED BUG lkf58113 */
...
/* Tds Maintenance: modified by yangzhi 00171698 End */
    /* Added by lkf58113 for AT Project, 2011-11-28, End*/
...
TAF_UINT32  At_QryTModePara(TAF_UINT8 ucIndex )
{
    ...
    /* Deleted by l00169177 20130829 DTS2013082901597, begin */
    /* Deleted by l00169177 20130829 DTS2013082901597, end */
    /* 查询当前TMODE的执行状态 */
    usLength =  (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,(TAF_CHAR *)pgucAtSndCodeAddr,
                                       (TAF_CHAR*)pgucAtSndCodeAddr, "%s:",
                                       g_stParseContext[ucIndex].pstCmdElement->pszCmdName);
...
VOS_UINT32  At_QryFlnaPara(VOS_UINT8 ucIndex)
{
    ...
    /*当前不为非信令模式*/
    if (AT_TMODE_FTM != g_stAtDevCmdCtrl.ucCurrentTMode)    /* FIXED BUG lkf58113 */
    {
        return AT_DEVICE_MODE_ERROR;
    }
...
TAF_UINT32  At_QryFChanPara(TAF_UINT8 ucIndex )
{
    ...
    if ((AT_RAT_MODE_FDD_LTE == g_stAtDevCmdCtrl.ucDeviceRatMode)
      ||(AT_RAT_MODE_TDD_LTE == g_stAtDevCmdCtrl.ucDeviceRatMode))
    {
        /* return atQryFFCHANSPara(ucIndex); */ /* lkf58113 @ 20111102 */
...
{
    /* Added by w00181244 for AT Project, 2011-10-15, Begin*/
    if(TAF_SUCCESS == AT_FillAndSndAppReqMsg(gastAtClientTab[ucIndex].usClientId,
                                            gastAtClientTab[ucIndex].opId,
    ...
    /* Added by w00181244 for AT Project, 2011-10-15, End*/
}
...
VOS_UINT32  At_QryQosPara(VOS_UINT8 ucIndex )
{
    ...
    {
        /* Modify by w00199382 for DTS2012031306141, 2012-03-19, Begin   */
        usLength = (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN, (VOS_CHAR *)pgucAtSndCodeAddr,
                                          (VOS_CHAR *)pgucAtSndCodeAddr, "%s:%d",
                                          g_stParseContext[ucIndex].pstCmdElement->pszCmdName,
                                          AT_QOS_TRAFFIC_CLASS_SUBSCRIBE);

        /* Modify by w00199382 for DTS2012031306141, 2012-03-19, End   */
...
{
    /* Modified by z60575 for DTS2012051806821, 2012-5-18, begin */
    ...
    /* Modified by z60575 for DTS2012051806821, 2012-5-18, end */
    VOS_UINT8                           ucLoop;
...
/* Added by f62575 for DTS2012071402176, 2012-07-28, Begin   */
/*****************************************************************************
...
/* Added by f62575 for DTS2012071402176, 2012-07-28, End   */
    ...
    VOS_UINT16                         *pusCsdValue)
{
    ...
    /* Modified by f62575 for DTS2012071402176, 2012-07-28, Begin   */
    /* 判断WIFI相关的NV项是否为默认值 */
    ulRet = AT_GetWifiNvValue(pusCsdValue);

    return ulRet;
    /* Modified by f62575 for DTS2012071402176, 2012-07-28, End     */
}
...
)
{
    ...
    /* Modified by w00199382 for DTS2012042803139,2012-3-5,Begin */
    ...
    /* Modified by w00199382 for DTS2012042803139,2012-3-5,End */
...
)
{
    ...
    {
        case AT_IPV6_CAPABILITY_IPV4_ONLY:
            /* Modified by A00165503 for DTS2011101405452, 2011-10-15, begin */
            enIpv4Status = AT_NdisGetConnStatus(AT_NdisGetState(TAF_PDP_IPV4));
            /* Modified by A00165503 for DTS2011101405452, 2011-10-15, end */
...
)
{
    ...
    /* Modified by f00179208 for DTS2012031400837, 2012-03-20 Begin */
    ...
    gstAtSendData.usBufLen = usLength;
    /* Modified by f00179208 for DTS2012031400837, 2012-03-20 End */
...
{
    /*Modified by f00179208 for PS Project, 2011-12-21, Begin */
    ...
    /*Modified by f00179208 for PS Project, 2011-12-21, End */
}
...
VOS_UINT32 AT_QryWiFiKeyPara(VOS_UINT8 ucIndex)
{
    ...
    /* Modify by w00199382 for DTS2012030804030, 2012-3-13, Begin   */
    ...
    /* Modify by w00199382 for DTS2012030804030, 2012-3-13, End   */
...
)
{
    ...
    /* Added by f62575 for DTS2012051007817, 2012-5-10, begin */
    ...
    /* Added by f62575 for DTS2012051007817, 2012-5-10, end */
    /* Added by t00212959 for DTS2012103101740, 2012-11-12, end */
...
VOS_UINT32 AT_QryProdNamePara(VOS_UINT8 ucIndex)
{
    ...
    /* Modified by l60609 for DTS2011080502346, 2011-08-08 Begin */
    ulRet = NV_ReadEx(MODEM_ID_0, en_NV_Item_PRODUCT_ID,
                    &stProductId,
                    sizeof(stProductId.ulNvStatus) + sizeof(stProductId.aucProductId));
    /* Modified by l60609 for DTS2011080502346, 2011-08-08 End */
...
{
    /* Modified by z60575 for DTS2013073103769, 2013-8-2, begin */
    VOS_UINT32                          ulRet;
    VOS_UINT16                          usAntState;
    /* Modified by z60575 for DTS2013073103769, 2013-8-2, end */
...
VOS_UINT32 AT_QrySARReduction(VOS_UINT8 ucIndex)
{
    ...
    MODEM_ID_ENUM_UINT16                enModemId;
    /* Added by z60575 for DSDA_SAR DTS2013060307614, 2013-5-30 end */
    ...
    /* Modified by z60575 for DSDA_SAR DTS2013060307614, 2013-5-30 begin */
    ...
    /* Modified by z60575 for DSDA_SAR DTS2013060307614, 2013-5-30 end */
...
/* Added by y00213812 for AP-Modem Personalisation Project, 2012/04/10, end */
/* Modified by z00161729 for V7R1C50 CSFB&PPAC&ETWS&ISR, 2012-4-1, begin */
#if (VOS_WIN32 == VOS_OS_VER)
/*****************************************************************************
...
/* Modified by z00161729 for V7R1C50 CSFB&PPAC&ETWS&ISR, 2012-4-1, end */
...
VOS_UINT32 At_QryCerssiPara(VOS_UINT8 ucIndex)
{
    ...
    VOS_UINT32                          ulResult;
    /* Modified by w00281933 for Service_State_Optimize_PhaseI, 2014-11-28, begin */

    PS_MEM_SET(&stCerssiInfoReq, 0, sizeof(stCerssiInfoReq));
    /* Modified by w00281933 for Service_State_Optimize_PhaseI, 2014-11-28, end */
...
VOS_UINT32 AT_QryCLteRoamAllowPara(VOS_UINT8 ucIndex)
{
    ...
    /* Modified by z00234330 for DTS201300600336, 2013-10-09, begin */
    ...
    /* Modified by z00234330 for DTS201300600336, 2013-10-09, end */
