 ...
 ******************************************************************************
  ...
  3.日    期   : 2008年03月03日
    作    者   : s46746
    修改内容   : 根据问题单号AT2D02570
  4.日    期   : 2013年07月08日
    作    者   : Y00213812
    修改内容   : VoLTE_PhaseI 项目，PDP EVENT结构变更，修改本文件中所有涉及的代码，
                 重构部分函数(见函数修改历史)。
...
/* Added by S62952 for DTS2012032802023, 2012-06-12, Begin */
/* IP*/
VOS_UINT32                              g_ulIpAddr = 0;
/* Added by S62952 for DTS2012032802023, 2012-06-12, End */
...
/* Added by f00179208 for PS Project, 2012-01-12, begin */
/******************************************************************************
...
/* Added by f00179208 for PS Project, 2012-01-12, End */
...
/* Added by l00198894 for PS Project, 2012/01/09, begin */
/*****************************************************************************
...
/* Added by l00198894 for PS Project, 2012/01/09, end */
...
/* Added by A00165503 for DTS2012032802023, 2012-06-12, Begin */
/*****************************************************************************
...
/* Added by A00165503 for DTS2012032802023, 2012-06-12, End */
...
)
{
    ...
    else
    {
        ...
        /* Added by A00165503 for HiLink IPv6, 2011-07-20, begin */
        if (AT_APP_USER == ucUserType)
        {
            AT_AppPdpAddrProc(&g_stAtAppPdpEntity, &stConfig, pstEvent);
        }
        /* Added by A00165503 for HiLink IPv6, 2011-07-20, end */

        /* Deleted by A00165503 for DTS2013110900839, 2013-11-11, Begin */
        /* 删除AT_USBCOM_USER处理分支 */
        /* Deleted by A00165503 for DTS2013110900839, 2013-11-11, End */
    }
...
)
{
   ...
   /* Modified by A00165503 for C50 IPv6 Project, 2012-12-18, Begin */
    ...
    /* Modified by A00165503 for C50 IPv6 Project, 2012-12-18, End */
...
)
{
    ...
    /* Added by Y00213812 for DTS2012111907501, 2012-11-20, begin */
...
)
{
    ...
    /* Modified by l60609 for DTS2012050800626,2012-5-15,Begin */
    ...
    /* Modified by l60609 for DTS2012050800626,2012-5-15,End */
}
...
{
    /* Modified by A00165503 for DTS2012072806289, 2012-07-28, Begin */
    if (AT_NDIS_PDNCFG_CNF_FAIL == pstNdisPdnInfoCfgCnf->enResult)
    {
        AT_ERR_LOG("AT_RcvNdisPdnInfoRelCnf: Failed.");
    }
    /* Modified by A00165503 for DTS2012072806289, 2012-07-28, End */
...
/* Added by A00165503 for DTS2012072806289, 2012-07-28, Begin */
/*****************************************************************************
...
/* Added by A00165503 for DTS2012072806289, 2012-07-28, End */
...
)
{
    ...
    /* Modified by A00165503 for PS DOMAIN ERRORCODE, 2012-08-22, End */
    {
        if (TAF_PDP_IPV4 == pstEvent->stPdpAddr.enPdpType)
        {
            ...
            {
                /* 查找一个未激活的CID进行激活 */
                /* Modified by c00173809 for PS Project,2011-12-17,Begin */
                ...
                /* Modified by c00173809 for PS Project,2011-12-17,End */
            }
        }
...
)
{
    ...
    /* Added by f00179208 for DTS2015031000157, 2015-03-11 begin */
    /* 获取第三PCSCF地址 */
    ...
    /* Added by f00179208 for DTS2015031000157, 2015-03-11 end */
...
)
{
    ...
    {
        /* Modified by Y00213812 for DTS2012111907501, 2012-11-20, begin */
        /* Added by A00165503 for PS DOMAIN ERRORCODE, 2012-08-22, Begin */
        ul3gppSmCause = AT_Get3gppSmCauseByPsCause(enCause);
        /* Added by A00165503 for PS DOMAIN ERRORCODE, 2012-08-22, End */
        /* Modified by Y00213812 for DTS2012111907501, 2012-11-20, end */
        ...
        switch (ucPdpType)
        {
            ...
            case TAF_PDP_IPV4V6:
                ...
                /* Modified by S62592 for DTS2011070103435, 2011-07-04, begin */
                usLength  = (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN, (VOS_CHAR*)pgucAtSndCodeAddr, (VOS_CHAR*)pgucAtSndCodeAddr + usLength,
                                            "%s^NDISSTAT:%d,%d,,\"%s\"%s",
                                            gaucAtCrLf,
                                            enStat,
                                            ...
                                            aucAtStrIpv4,
                                            gaucAtCrLf);
                /* Modified by S62592 for DTS2011070103435, 2011-07-04, end */
...
{
    /* Modified by Y00213812 for V9R1 IPv6&TAF/SM Project, 2013-05-07, Begin */
    AT_PDP_ENTITY_STRU                 *pstNdisPdpEntity;
    ...
    /* Modified by Y00213812 for V9R1 IPv6&TAF/SM Project, 2013-05-07, End */
    ...
    if (TAF_PDP_IPV4V6 == g_enAtFirstNdisActPdpType)
    {
        ...
        else if ( TAF_PDP_IPV4 == g_enAtNdisActPdpType )
        {
            ...
            /* Modified by A00165503 for DTS2011101405464, 2011-10-15, begin */
            /* Modified by Y00213812 for V9R1 IPv6&TAF/SM Project, 2013-05-07, Begin */
            if ( (AT_PDP_STATE_IDLE == AT_NdisGetState(TAF_PDP_IPV6))
            ...
            /* Modified by A00165503 for DTS2011101405464, 2011-10-15, end */
        }
        else
        {
...
)
{
    ...
    /* 通知TTF去激活相应的RAB */
    /* Modified by A00165503 for AT Project, 2011-10-20, begin */
    AT_SendNdisRelReq(pstEvent);
    /* Modified by A00165503 for AT Project, 2011-10-20, end */
...
/* Added by A00165503 for DTS2012051708455, 2012-05-18, Begin */
/*****************************************************************************
...
/* Added by A00165503 for DTS2012051708455, 2012-05-18, End */

/* Added by A00165503 for DTS2012042104301, 2013-01-14, Begin */
/*****************************************************************************
...
/* Added by A00165503 for DTS2012042104301, 2013-01-14, End */
...
)
{
    ...
    /* Modified by A00165503 for DTS2012062900707, 2012-06-30, Begin */
    if ( (ulIpv6LanAddrLen < TAF_IPV6_ADDR_LEN )
      || (ulPrefixByteLen > AT_IPV6_ADDR_PREFIX_BYTE_LEN) )
    {
        return VOS_ERR;
    }
    /* Modified by A00165503 for DTS2012062900707, 2012-06-30, End */
...
)
{
    ...
    /* Modified by l60609 for DTS2012051002464, 2012-5-11, begin */
    ...
    usLength = (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                      ...
                                      gaucAtCrLf);
    /* Modified by l60609 for DTS2012051002464, 2012-5-11, end */
    /* Modified by A00165503 for DTS2012042104301, 2013-01-14, Begin */
    At_SendResultData(AT_APP_GET_USR_PORT_INDEX(), pgucAtSndCodeAddr, usLength);
    /* Modified by A00165503 for DTS2012042104301, 2013-01-14, End */
...
/* Added by A00165503 for DTS2012062900707, 2012-06-30, Begin */
/*****************************************************************************
...
/* Added by A00165503 for DTS2012062900707, 2012-06-30, End */
...
VOS_UINT32 AT_SendRnicIpv6ActInd(VOS_UINT8 ucRmNetId)
{
    ...
    /* Added by l00227485 for DSDA PhaseII, 2012-12-27, begin */
    pstMsg->ucRmNetId       = ucRmNetId;
    /* Added by l00227485 for DSDA PhaseII, 2012-12-27, end */
...
)
{
    ...
    /* Modified by A00165503 for DTS2012041806921, 2012-04-21, Begin */
    else
    {
        /* 根据原因值, 通知APP拨号结果 */
        switch (pstEvent->enCause)
    ...
    /* Modified by A00165503 for DTS2012041806921, 2012-04-21, End */
...
)
{
    ...
    /* Modified by Y00213812 for V9R1 IPv6&TAF/SM Project, 2013-05-07, End */
    {
        /* Modified by A00165503 for DTS2012061408511, 2012-06-16, Begin */
        /* 检查IPv6连接是否已经存在, 如果不存在, 发起IPv6类型的PDP激活 */
        if (AT_PDP_STATE_IDLE == AT_AppGetPdpState(TAF_PDP_IPV6))
        {
            /* 发起IPv6类型的PDP激活请求 */
        ...
        /* Modified by A00165503 for DTS2012061408511, 2012-06-16, End */
...
)
{
    ...
    /* Modified by l00227485 for DSDA PhaseII, 2012-12-22, begin */
    ...
    /* Modified by l00227485 for DSDA PhaseII, 2012-12-22, end */
...
/* Added by f00179208 for V3R3 PPP RPOJECT 2013-06-05, Begin */
/*****************************************************************************
...
/* Added by f00179208 for V3R3 PPP RPOJECT 2013-06-05, End */
...
)
{
    ...
    /* Added by A00165503 for DTS2013120310013, 2014-01-14, Begin */
    if (AT_CMD_PS_DATA_CALL_END_SET == gastAtClientTab[ucIndex].CmdCurrentOpt)
    {
        return;
    }
    /* Added by A00165503 for DTS2013120310013, 2014-01-14, End */
...
/* Added by f00179208 for DTS2012052205142, 2012-05-22 Begin */
...
#endif
/* Added by f00179208 for DTS2012052205142, 2012-05-22 End */
...
{
    VOS_UINT32                          ulRet;
    /* Added by l00227485 for DSDA PhaseII, 2012-12-22, begin */
    MODEM_ID_ENUM_UINT16                enModemId;
    ...
    /* Added by l00227485 for DSDA PhaseII, 2012-12-22, end */
...
)
{
    ...
    {
        /* Modified by A00165503 for DTS2014050900349, 2014-05-09, Begin */
        ...
        /* Modified by A00165503 for DTS2014050900349, 2014-05-09, End */
    }
...
)
{
    ...
    /* Added by A00165503 for C50 IPv6 Project, 2012-12-18, Begin */
    pstAtDipcPdpAct->enBearerType    = enBearerType;
    /* Added by A00165503 for C50 IPv6 Project, 2012-12-18, End */
...
VOS_VOID AT_PS_ProcIpv6RaInfo(TAF_PS_IPV6_INFO_IND_STRU *pstRaInfoNotifyInd)
{
    ...
    /* 获取到IPv6地址前缀后, 上报已连接结果^DCONN */
    /* Modified by A00165503 for DTS2013011803709, 2013-01-21, Begin */
    ...
    /* Modified by A00165503 for DTS2013011803709, 2013-01-21, End */
