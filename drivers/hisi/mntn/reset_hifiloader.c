 ...
 ******************************************************************************
  ...
  2.日    期   : 2013年5月22日
    作    者   : 卢彦胜 00212112
    修改内容   : 解决DTS2013051800980，去掉函数定义以及调用
...
int drv_hifi_load_sec(void *img_buf, unsigned int* share_mem)
{
    ...
    if (NULL == img_buf || NULL == share_mem) {
    ...
    for (i = 0; i < head->sections_num; i++) {
        ...
        printk("drv_hifi_load_sec00:iviraddr is 0x%pK, phy addr is 0x%x.\n",(void*)iviraddr, head->sections[i].des_addr);
        if ((unsigned char)DRV_HIFI_IMAGE_SEC_LOAD_STATIC == head->sections[i].load_attib) {
            ...
            printk("drv_hifi_load_sec02:dynamic step1, head = 0x%pK dynamic_sec = 0x%pK\n", (void*)head,  (void*)dynamic_sec);
            /* copy the sections */
            ...
            printk("drv_hifi_load_sec02:iviraddr is 0x%pK phy addr is 0x%pK.\n",(void*)iviraddr_src, (void*)dynamic_sec->sec_source_addr);
...
int drv_hifi_read_image(void *img_head, void **img_buf)
{
    ...
    *buff = vmalloc(size);
    ...
    printk(KERN_INFO"RESET RESET: read hifi bin to virt addr 0x%pK, size = %d\n", (void*)(*buff), size);
...
int  execute_load_hifi(int safe_load, unsigned long share_mem)
{
    ...
    /*DTS2013051604733, 20130531,l00212112,begin*/
    share_memory = ioremap_wc(share_mem,HIFI_SHARE_ADDR_LENGTH);
    /*DTS2013051604733, 20130531,l00212112,end*/
    ...
    printk(KERN_INFO"RESET LOADHIFI: Enter execute_load_hifi, safe = %d, share_mem = 0x%pK\n", safe_load, share_memory);
