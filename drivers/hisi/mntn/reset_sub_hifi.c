...
sreset_mgr_LLI * reset_do_regcbfunc(sreset_mgr_LLI *plink, const char *pname, pdrv_reset_cbfun pcbfun, int userdata, int priolevel)
{
    ...
    {
        printk(KERN_ERR "%s: fail in ccore reset regcb,fail, name 0x%pK, cbfun 0x%pK, prio %d\n", __FUNCTION__, \
...
/* DTS:DTS2015031310280 Hifi_crash modem_crash notify logservice start 2015-3-16 */
/*****************************************************************************
...
/* DTS:DTS2015031310280 Hifi_crash modem_crash notify logservice end 2015-3-16 */
/*caixi 7*/
...
int hifireset_task(void *arg)
{
...
#endif
    {
...
#ifdef _DRV_LLT_
#else
        /*waiting for mcu return DTS2013100903550  l00249396 2013-10-14 start*/
        ...
        /*waiting for mcu return DTS2013100903550  l00249396 2013-10-14 end*/
...
static int drv_hifireset_cbfun(DRV_RESET_CB_MOMENT_E eparam, int userdata)
{
    ...
    if (MDRV_RESET_CB_BEFORE == eparam)
    {
        ...
        else
        {
            ...
            printk(KERN_INFO"RESET LOG %s: hifi_nv_phy_addr  = 0x%x, hifi_nv_vir_addr = 0x%pK, (*hifi_nv_vir_addr) = 0x%x\n",
...
void reset_info_show_hifi(void)
{
    ...
    while (NULL != phead)
    {
        printk(KERN_INFO "%s: hifi reset: name:%s cbfun:0x%pK data:%d priolevel:%d\n", __FUNCTION__, phead->cbfuninfo.name,
