    ...
    VOS_UINT32                          ulLen)
{
    ...
    for( i = 0; i < ulLen; i++ )
    {
        ...
        if ( HDLC_SUCC == ulResult )
        {
            ...
            if (SCM_DATA_TYPE_TL == ucGutlType)
            {
...
VOS_UINT32 CBTSCM_SoftDecodeReqRcvTaskInit(VOS_VOID)
{
    ...
    if (VOS_NULL_PTR == g_stCbtScmDataRcvTaskCtrlInfo.rngOmRbufId)
    {
        ...
        VOS_MemFree(ACPU_PID_OM, g_stCbtScmHdlcSoftDecodeEntity.pucDecapBuff);
