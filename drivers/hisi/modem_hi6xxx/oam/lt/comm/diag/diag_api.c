...
VOS_UINT32 DIAG_LogReport(VOS_UINT32 ulModuleId, VOS_UINT32 ulPid, VOS_CHAR *cFileName, VOS_UINT32 ulLineNum, VOS_CHAR* pszFmt, ...)
{
    ...
    /* 只记录调用次数*/
    DIAG_DEBUG_SDM_FUN(EN_SDM_DIAG_PRINTFV, ulModuleId, 0, 0);

    /*检查DIAG是否初始化且HSO是否连接上*/
    if(!DIAG_IS_CONN_ON)
    {
        DIAG_DEBUG_SDM_FUN(EN_SDM_DIAG_PRINTFV_ERR, ERR_MSP_NO_INITILIZATION, ulModuleId, 1);
        return ERR_MSP_NO_INITILIZATION;
    }
    ...
    /*获取模块开关状态*/
    ulLevelFilter = DIAG_GET_PRINTF_LEVEL(ulModuleId);
#if 1
    ulRst = diag_GetPrintCfg(ulPID,ulLevelFilter);
    if(ERR_MSP_SUCCESS != ulRst)
    {
        DIAG_DEBUG_SDM_FUN(EN_SDM_DIAG_PRINTFV_ERR, ERR_MSP_UNAVAILABLE, ulPID, 2);
        return ERR_MSP_UNAVAILABLE;
    }
#endif

    /*给HSO的打印字符串形式如下:cFileName[ulLineNum]data。HSO根据中括号[]去截取相应的信息*/
    /*lint -save -e539*/
	ulParamLen = VOS_nsprintf(ucDiagPrintData,DIAG_PRINTF_MAX_LEN,"%s[%d]",cFileName,ulLineNum);
    /*lint -restore*/
    /* coverity[cond_at_least] */
    if(ulParamLen > DIAG_PRINTF_MAX_LEN)
    {
        DIAG_DEBUG_SDM_FUN(EN_SDM_DIAG_PRINTFV_ERR, ERR_MSP_FAILURE, ulPID, 11);
    }
    ...
    if(ulParamLen > DIAG_PRINTF_MAX_LEN)
    {
        DIAG_DEBUG_SDM_FUN(EN_SDM_DIAG_PRINTFV_ERR, ERR_MSP_FAILURE, ulPID, 12);
    }
    va_end(arg);
