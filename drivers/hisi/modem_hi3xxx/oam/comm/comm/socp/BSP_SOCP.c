...
/* Added by L00256032 for V8R1 OM_Optimize Project, 2014-05-20, begin */
/*****************************************************************************
 函 数 名  : SOCP_MsConvertToCycle
 功能描述  : 将ms为单位的延时时长，转换为SOC的计数单位(计数单位为0xFFFF个时钟周期，
             在150Mhz频率下约为436.9us: (0xFFFF * 1000 * 1000)/(150 * 1000 * 1000) = 0xFFFF / 150 ≈ 436.9us。
             转换算法: (DelayTimeInMs * 1000) / (0xFFFF / 150) = (DelayTimeInMs * 1000 * 150) / 0xFFFF
                        = DelayTimeInMs * ((1000 * 150) / 0xFFFF) = DelayTimeInMs * 2.289 = DelayTimeInMs * 2289 / 1000
 输入参数  : ulDelayTimeInMs         -- 以ms为单位的延迟时长 (AT^LOGCFG命令的<timeout>参数)
 输出参数  :
 返 回 值  : 转换后的以SOC的计数单位为单位的数值
 调用函数  :
 被调函数  :
...
/* added by z00212940 for LOG2.0 at 2012-12-20 end */
...
/*****************************************************************************
 函 数 名  : SocpEncDstTaskChannel
 功能描述  : 处理编码目的中断的自处理任务入口
 输入参数  : ulSocpEncDestCh  --- 编码目的通道号
             ulPara2          --- 暂不使用
             ulPara3          --- 暂不使用
             ulPara4          --- 暂不使用
 输出参数  : 无
...
/* Deleted by L00256032 for V8R1 OM_Optimize Project, 2014-05-22, begin */
#if 0
...
#endif
/* Deleted by L00256032 for V8R1 OM_Optimize Project, 2014-05-22, end */
...
VOS_VOID SocpCoreInit(VOS_VOID)
{
    ...
    /* Added by L00256032 for V8R1 OM_Optimize Project, 2014-05-20, end */
...
/* added by z00212940 for LOG2.0 at 2012-12-20 begin */
...
/* added by z00212940 for LOG2.0 at 2012-12-20 end */
    ...
    {
        /* Modified by L00256032 for V8R1 OM_Optimize Project, 2014-05-20, begin */
        /* 配置为NV项en_NV_ID_SOCP_SDLOG_CFG中配置的延迟时间 */
        SOCP_REG_WRITE(SOCP_REG_INTTIMEOUT, SOCP_MsConvertToCycle(pstcfg->overTime));
        /* Modified by L00256032 for V8R1 OM_Optimize Project, 2014-05-20, end */
    }
    else
    {
        SOCP_REG_WRITE(SOCP_REG_INTTIMEOUT, SOCP_TRANS_TIMEOUT_DEFAULT);    /* 超时传输中断超时配置 */
