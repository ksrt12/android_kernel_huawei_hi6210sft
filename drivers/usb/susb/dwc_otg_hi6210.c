...
#endif
/*END PN:DTS2015111011171, Added by s00258714, 2015/11/10*/
...
#endif
/* END PN:DTS2014082908201, Added by s00258714, 2014/9/3*/
...
int get_resource(struct hiusb_info *hiusb_info)
{
    ...
    usb_dbg("%s nrst addr:%pK value:0x%8.8x udelay:%d\n",
                __func__,
                hiusb_nrst[0].addr,
                hiusb_nrst[0].value,
                hiusb_nrst[0].udelay);

    usb_dbg("%s rst addr:%pK value:0x%8.8x udelay:%d\n",
                __func__,
                hiusb_rst[0].addr,
                hiusb_rst[0].value,
                hiusb_rst[0].udelay);
    ...
    if (np) {
        ...
        #endif
        /*BEGIN PN:DTS2015060206056, Added by s00258714, 2015/11/10*/
...
STATIC int detect_charger_type(void)
{
    ...
    if (peri_sctrl_stat2.reg.datcondet_sts == 0) {
        /* BEGIN PN:DTS2014082908201, Modified by l00189337, 2014/9/3*/
#ifdef CONFIG_CHARGER_TYPE_RECHECK
...
#endif
        /* END PN:DTS2014082908201, Modified by l00189337, 2014/9/3*/
        goto out;
    }
...
STATIC int hiusb_init_resource(struct lm_device *dev)
{
    ...
    hiusb_info->pm_qos_req.pm_qos_class = 0;/*DTS2013082604935 j00207786 2013-08-26 */
