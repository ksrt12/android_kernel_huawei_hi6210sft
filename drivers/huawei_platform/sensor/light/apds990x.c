 ...
 */
 ...
 /* BEGIN PN:DTS2014090405836,Modified by suzhanzheng, 2014/9/4*/
...
#endif
/* zkf55108 2011/10/26 modified begin */
			...
			if(err<0)
			{
			 if(apds990x_als_ps_init_flag==1)
			 {
			   if(!dsm_client_ocuppy(als_ps_dclient))
				 ...
				 /* BEGIN PN:DTS2014102405848 ,Modified by f00184246, 2014/10/25*/
				 dsm_client_record(als_ps_dclient,"[%s]i2c_status:%d,[irq gpio]num:%d,irq gpio status:%d.\n",__func__,err,data->apds_gpio, gpio_get_value(data->apds_gpio));
				  /* END PN:DTS2014102405848 ,Modified by f00184246, 2014/10/25*/
...
static void apds990x_change_ps_threshold(struct i2c_client *client)
{
    ...
    // als_ps_INFO("[ALS_PS]ps_data = %d min_proximity_value = %d pthreshold_h = %d pthresold_l= %d\n",data->psdata,min_proximity_value,pthreshold_h,pthreshold_l);
		...
		{
	         /*BEGIN PN:DTS2014083000691    , Added by f00184246, 2014/08/30 */
			 apds990x_i2c_write(client,APDS990x_PILTL_REG,FAR_THRESHOLD(threshold_value),APDS990X_I2C_WORD);
			 /*END PN:DTS2014083000691    , Added by f00184246, 2014/08/30 */
			loop--;
	    }
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1) {
		...
		if (data->enable_ps_sensor == 0) {
			...
			/* <DTS2012021801537 z00190171 2012/02/21 modify begin */
			apds990x_set_atime(client, 0xfa); /* 27.2ms */
			/* DTS2012021801537 z00190171 2012/02/21 modify end > */
			...
			/* END PN:DTS2014071407794   , Added by f00184246, 2014/07/14 */
			/* BEGIN PN:DTS2014072408688    , Added by f00184246, 2014/08/12 */
			if(1!=ps_phone)
			{
				//enable_irq(data->irq);
			    operate_irq(data,1,true);
			...
			/* END PN:DTS2014072408688    , Added by f00184246, 2014/08/12 */
		}
	}
	 else
	 {
		...
		/* BEGIN PN:DTS2014072408688    , Added by f00184246, 2014/08/12 */
		 /* BEGIN PN:DTS2014111903296, Added by f00184246, 2014/11/19 */
		if(1!=ps_phone)
		{
			operate_irq(data,0,true);
		}
	...
	case FB_BLANK_UNBLANK:
		...
		case FB_EARLY_EVENT_BLANK:
			...
			if (!this_light_data->enable_ps_sensor && this_light_data->enable_als_sensor) {
			    ...
			    /* BEGIN PN:DTS2014110404760 ,Modified by F00184246, 2014/11/08*/
				...
				/* END PN:DTS2014110404760 ,Modified by F00184246, 2014/11/08*/
...
{
 	/*BEGIN PN:DTS2014083000691    , Modified by f00184246, 2014/08/30 */
	...
	/*END PN:DTS2014083000691    , Modified by f00184246, 2014/08/30 */
