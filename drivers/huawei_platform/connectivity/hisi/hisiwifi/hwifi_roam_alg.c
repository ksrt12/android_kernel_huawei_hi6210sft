 ...
 * Description   :
 * Function List :
              find_candidate_ap_by_bssid
              hwifi_channel_index_to_band
              hwifi_roam_calc_avail_channel_capacity_to_utility
              hwifi_roam_is_in_ap_blacklist
              hwifi_roam_ssid_is_dismatch
              hwifi_roam_upadate_candidate_ap
              roam_select_candidate_ap
 * History       :
 * 1.Date        : 2013/1/15
 *   Author      : k53369
...
/* DTS2014062506177 HI1101 bug fix 2014-6-25 begin */
STATIC int32 get_wpa_cipher_suite(uint8 *ie, int32 ie_len, roam_wpa_cipher_suite_t *data)
{
    ...
    if (left >= 2)
    {
        ...
        /* DTS2015041406587 HI1101 bug fix zhanglu/z00217036 2015-4-15 begin */
        for (i = 0; i < count; i++)
        {
            ...
            /* DTS2015041406587 HI1101 bug fix zhanglu/z00217036 2015-4-15 end */
            pos += WPA_SELECTOR_LEN;
            left -= WPA_SELECTOR_LEN;
        }
    }
...
/* DTS2014062506177 HI1101 bug fix 2014-6-25 end */

/*
 * Prototype    : hwifi_roam_wpa_rsn_ie_is_mismatch
 * Description  : 判断RSN/WPA加密方式是否一样
 * Input        : cfg : addr of hi1101 cfg
                : beacon_ie_len : length of beacon_ie
                : beacon_ie : content of beacon_ie
 * Output       : None
 * Return Value : TRUE/FALSE
 * Calls        : null
 * Called By    : hwifi_roam_ie_is_mismatch
 *
 *   History        :
 *   1.Date         : 2014/6/7
 *     Author       : d00223710
...
/*
 * Prototype    : hwifi_roam_wapi_ie_is_mismatch
 * Description  : 判断WAPI加密方式是否一样
 * Input        : cfg : addr of hi1101 cfg
                : beacon_ie_len : length of beacon_ie
                : beacon_ie : content of beacon_ie
 * Output       : None
...
/*
 * Prototype    : hwifi_roam_open_ie_is_mismatch
 * Description  : 判断WAPI加密方式是否一样
 * Input        : cfg : addr of hi1101 cfg
                : beacon_ie_len : length of beacon_ie
                : beacon_ie : content of beacon_ie
...
/*
 * Prototype    : hwifi_roam_ie_is_mismatch
 * Description  : 判断加密方式是否一样
 * Input        : cfg : addr of hi1101 cfg
                : beacon_ie_len : length of beacon_ie
                : beacon_ie : content of beacon_ie
 * Output       : None
...
/*
 * Prototype    : calc_min_diff_rssi
 * Description  : 计算新旧AP漫游rssi最小的差值
 * Input        : struct roam_all_stru *pst_roam_info
                : int32 rssi_current
 * Output       : None
 * Return Value : min_diff_rssi
 * Calls        : null
 * Called By    : 
 *
 *   History        :
...
/*
 * Prototype    : calc_scan_interval
 * Description  : 计算新旧AP漫游扫描的间隔
 * Input        : struct roam_all_stru *pst_roam_info
                : int32 rssi_current
 * Output       : None
 * Return Value : min_scan_interval
 * Calls        : null
 * Called By    : 
 *
 *   History        :
...
/* DTS2014070308446  HI1101 bug fix 2014-7-4 begin */
    ...
    /* DTS2014092306945 HI1101 bug fix 20141008 begin */
    /* 记录漫游时选择目标AP的rssi */
    pst_roam_info->roam_fsm.target_ap_rssi = max_rssi;
    /* DTS2014092306945 HI1101 bug fix 20141008 end */
...
/* DTS2014073003055  HI1101 bug fix 20140806 begin */
/*
 * Prototype    : hwifi_roam_blacklist_bssid_clear
 * Description  : hwifi_roam_blacklist_bssid_clear: 1) if bssid is null, then return
                : 2) if bssid is not null, remove
                : the ap from the blacklist
 * Input        : pst_roam_info : all info of roam
                : bssid : addr of bssid
 * Output       : None
 * Return Value : void
