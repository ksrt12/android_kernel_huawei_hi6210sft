...
/*
 * Prototype    : update_roam_scan_info
 * Description  : 如果本次扫描没有扫描到相同ssid和加密方式的ap，则漫游扫描计数+1，按长时间漫游扫描策略
 *                否则，漫游扫描计数清0，按短漫游扫描策略
 * Input        :
 * Output       : None
 * Return Value : void
 * Calls        :
...
/*
 * Prototype    : find_can_roamed_ap
 * Description  : 查找周围是否有相同ssid的AP可供漫游
 * Input        :
 * Output       : None
 * Return Value : void
 * Calls        :
...
/* DTS2014062506177 HI1101 bug fix 2014-6-25 end */
 ...
 /*
 * Prototype    : inform_single_beacon_frame
 * Description  :
 * Input        :
 * Output       : None
 * Return Value : void
...
/* DTS2015011201821  Hi110x bug fix zhanglu z00217036  2015/01/13 begin*/
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 39))
    struct timespec ts;
#else
    struct timeval tv;
#endif
/* DTS2015011201821  Hi110x bug fix zhanglu z00217036  2015/01/13 end*/
...
/* DTS2015011201821  Hi110x bug fix zhanglu z00217036  2015/01/13 end*/
    ...
    /* add by zhanglu  DTS2013070108579  */
...
/* DTS2014112104111 Hi1101 bug fix hwx212164 2014/01/14 begin */
    timeout = 2 * HZ;
/* DTS2014112104111 Hi1101 bug fix hwx212164 2014/01/14 end */
...
int for_each_scanned_bss(struct scanned_bss_mgmt *bss_mgmt, SCAN_CB_F cb, void *data)
{
 ...
 /*
 * Prototype    : __find_scanned_bss_by_bssid
 * Description  :
 * Input        : struct scanned_bss_mgmt *bss_mgmt
 * Output       : None
 * Return Value : void
 * Calls        :
 * Called By    :
...
STATIC struct scanned_bss_info* __find_scanned_bss_by_bssid(struct scanned_bss_mgmt *bss_mgmt, uint8* bssid)
{
 ...
 /*
 * Prototype    : find_scanned_bss_by_bssid
 * Description  :
 * Input        : struct scanned_bss_mgmt *bss_mgmt
 * Output       : None
 * Return Value : void
...
struct scanned_bss_info* find_scanned_bss_by_bssid(struct scanned_bss_mgmt *bss_mgmt, uint8* bssid)
{
 ...
 /*
 * Prototype    : __find_scanned_bss_by_ssid
 * Description  :
 * Input        : struct scanned_bss_mgmt *bss_mgmt
 * Output       : None
 * Return Value : void
...
STATIC struct scanned_bss_info* __find_scanned_bss_by_ssid(struct scanned_bss_mgmt *bss_mgmt, uint8* ssid, int32 len)
{
 ...
 /*
 * Prototype    : find_scanned_bss_by_ssid
 * Description  :
 * Input        : struct scanned_bss_mgmt *bss_mgmt
 * Output       : None
 * Return Value : void
...
int update_bss_info(struct scanned_bss_mgmt *bss_mgmt, struct scanned_bss_info* bss)
{
    ...
    if (NULL != old_bss)
    {
        ...
        /* DTS2013112802048 Hi110x bug fix by l00137985 2013/12/23 begin */
        if(old_bss->signal > bss->signal)
        {
		    /* DTS2014032901553 Hi110x fix by hwx212164 2014/04/14 beigin */
	        bss->signal = old_bss->signal;
			/* DTS2014032901553 Hi110x fix by hwx212164 2014/04/14 end */
		}
				
		HWIFI_DEBUG("new_signal:%d, old_signal:%d \n",  bss->signal, old_bss->signal);
	    /* DTS2013112802048 Hi110x bug fix by l00137985 2013/12/23 end */
...
/*
 * Prototype    : clean_scanned_bss_immediately
 * Description  :
 * Input        :
 * Output       : None
 * Return Value : None
