...
/*add by g00260377 for security issue @DTS2015110502810 2015.11.03 end*/
...
static int s_vpu_major = 0;
static struct cdev s_vpu_cdev = {0};

static vpu_clk s_vpu_clk = {0};
...
static void __iomem *s_vpu_qos_virt_addr = NULL;
...
static u32 s_vpu_reg_store[MAX_NUM_VPU_CORE][64] = {0};
static u32 s_run_index  = 0;
static u32 s_run_codstd = 0;

static u32 s_vpu_frame_clock_state  = 0;
static u32 s_vpu_suspend_state      = 0;
static struct wake_lock s_vpu_wakelock = {0};
...
{
    vpudrv_instanace_list_t *vil = NULL;
    vpudrv_instanace_list_t *n   = NULL;
    vpudrv_instance_pool_t *vip  = NULL;
    void *vip_base = NULL;
	...
	list_for_each_entry_safe(vil, n, &s_inst_list_head, list)
	{
		if (vil->filp == filp) {
			...
			//vpu_logd("vpu_free_instances detect instance crash instIdx=%d, coreIdx=%d, vip_base=%pK, instance_pool_size_per_core=%d\n", (int)vil->inst_idx, (int)vil->core_idx, vip_base, (int)instance_pool_size_per_core);
...
{
    vpudrv_buffer_pool_t *pool =NULL;
    vpudrv_buffer_pool_t *n = NULL;
...
{
    /*Added by s00216212 for DTS2015031701148, modify for vpu irq error start*/
    if (s_vpu_drv_context.open_count == 0)
    {
        return IRQ_HANDLED;
    }
    /*Added by s00216212 for DTS2015031701148, modify for vpu irq error end*/
...
static long vpu_ioctl(struct file *filp, u_int cmd, u_long arg)
{
    ...
    if (_IOC_NR(cmd) > VDI_IOCTL_MAXNR) {
        return -ENOTTY;
    }
            ...
            else
			{
				...
				else
                {
                    ...
                    if (s_ion_dev == NULL)
                    {
                       ...
                       //vpu_loge("[DEBUG, not ERROR] s_ion_dev = %pK.\n", s_ion_dev);
                    }
                    ...
                    //vpu_loge("[DEBUG, not ERROR] s_ion_instance_pool_client = %pK.\n", s_ion_instance_pool_client);
                    ...
                    //vpu_loge("[DEBUG, not ERROR] s_ion_instance_pool_handle = %pK.\n", s_ion_instance_pool_handle);
...
ERROR_INSTANCE_MEMORY_ALLOC:
                    if (0 != ret)
                    {
                        if ((!IS_ERR((void *)s_instance_pool.base)) && (s_instance_pool.base != 0))
                        {
                            //vpu_loge("s_instance_pool.base = 0x%lx s_ion_instance_pool_client = %pK s_ion_instance_pool_handle = %pK",
                        ...
                        if ((!IS_ERR(s_ion_instance_pool_handle)) && (s_ion_instance_pool_handle != NULL))
                        {
                            //vpu_loge("s_ion_instance_pool_client = %pK s_ion_instance_pool_handle = %pK",
                        ...
                        if ((!IS_ERR(s_ion_instance_pool_client)) && (s_ion_instance_pool_client != NULL))
                        {
                            //vpu_loge("s_ion_instance_pool_client = %pK", s_ion_instance_pool_client);
    ...
    case VDI_IOCTL_RESERVED_MEMORY_ALLOC:
        {
            ...
            //vpu_loge("[DEBUG, not ERROR] s_ion_reserved_mem_client= %pK.\n", s_ion_reserved_mem_client);
            ...
            //vpu_loge("[DEBUG, not ERROR] s_ion_reserved_mem_handle = %pK.\n", s_ion_reserved_mem_handle);
...
ERROR_RESERVED_MEMORY_ALLOC:
            ...
            if (0 != ret)
            {
                ...
                if ((!IS_ERR(s_ion_reserved_mem_handle)) && (s_ion_reserved_mem_handle != NULL))
                {
                    //vpu_loge("s_ion_reserved_mem_client = %pK s_ion_reserved_mem_handle = %pK\n", s_ion_reserved_mem_client, s_ion_reserved_mem_handle);
                ...
                if ((!IS_ERR(s_ion_reserved_mem_client)) && (s_ion_reserved_mem_client != NULL))
                {
                    //vpu_loge("s_ion_reserved_mem_client = %pK\n", s_ion_reserved_mem_client);
...
static int vpu_release(struct inode *inode, struct file *filp)
{
    ...
    /*added for DTS2014092505534 by y00251056 start*/
    vpu_clk_enable();
    /*====================END======================*/
    /*Added by s00216212 for DTS2015031701148, modify for vpu irq error start*/
    ...
    if (s_instance_pool.size != 0)
    {
        if ((!IS_ERR((void *)s_instance_pool.base)) && (s_instance_pool.base != 0))
        {
            //vpu_logi(" ion_unmap_kernel:s_ion_instance_pool_client = %pK, s_ion_instance_pool_handle = %pK, s_instance_pool.base = 0x%lx\n",
        ...
        if ((!IS_ERR(s_ion_instance_pool_handle)) && (s_ion_instance_pool_handle != NULL))
        {
            //vpu_logi("ion_free: s_ion_instance_pool_client = %pK, s_ion_instance_pool_handle = %pK\n",
                //s_ion_instance_pool_client, s_ion_instance_pool_handle);
        ...
        if ((!IS_ERR(s_ion_instance_pool_client)) && (s_ion_instance_pool_client != NULL))
        {
            //vpu_logi("ion_client_destroy: ion_instance_pool_client = %pK\n", s_ion_instance_pool_client);
    ...
    if (1 == s_vpu_systemMMU_support)
    {
        ...
        if ((!IS_ERR(s_ion_reserved_mem_handle)) && (s_ion_reserved_mem_handle != NULL))
        {
            //vpu_logi("s_ion_reserved_mem_client = %pK, s_ion_reserved_mem_handle = %pK\n", s_ion_reserved_mem_client, s_ion_reserved_mem_handle);
        ...
        if ((!IS_ERR(s_ion_reserved_mem_client)) && (s_ion_reserved_mem_client != NULL))
        {
            //vpu_logi("s_ion_reserved_mem_client = %pK\n", s_ion_reserved_mem_client);
...
static int vpu_map_to_physical_memory(struct file *fp, struct vm_area_struct *vm)
{
    ...
    if (1 == s_vpu_systemMMU_support)
    {
        if (vm->vm_pgoff == (s_instance_pool.phys_addr>>PAGE_SHIFT))
        {
            ...
            if ((NULL == s_ion_instance_pool_client) || (NULL == s_ion_instance_pool_handle))
            {
                return -EAGAIN;
            }
            else
        ...
        /*Modify by g00260377 for security issue @DTS2015110502810 2015.11.03 begin*/
        ...
        /*Modify by g00260377 for security issue @DTS2015110502810 2015.11.03 end*/
...
static int vpu_probe(struct platform_device *pdev)
{
    ...
    if (res) 	// if platform driver is implemented
    {
        ...
        //vpu_logi("vpu reg phy addr=0x%x, reg virtual base=%pK\n", s_vpu_reg_phy_addr , s_vpu_reg_virt_addr);
    }
    else
    {
        err = -ENODEV;
